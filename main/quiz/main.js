...
function butt(optValue, answerValue) {
    if (optValue == answerValue) {
        marks++
        console.log(marks)
    }
    next()
...
function startQuiz() {
    ...
    for (var i = 0; i < arr[score].option.length; i++) {
     ...
     <button type="button" class="btn btn1 text-white darkBackground fs-4 py-3 mt-3 w-75" onclick="butt()"> ${arr[score].option[i]} </button> </div>`
    }
...
// ${arr[score].option[i]},${arr[score].correctoption}